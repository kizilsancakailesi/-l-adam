...
def bul(client, message):
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    m = message.reply('🔎 Şarkı yükleniyooo...')
...
async def ytmusic(client,message: Message):
    ...
    await client.send_video(message.chat.id, video = open(file_stark, "rb"), duration = int(ytdl_data["duration"]), file_name = str(ytdl_data["title"]), thumb = sedlyf, caption = capy, supports_streaming = True , progress=progress, progress_args=(pablo, c_time, f'`loading {urlissed} YouTube Music'ten Şarkı!`', file_stark))
    await pablo.delete()
    is_downloading = False
